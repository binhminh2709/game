...
public class WordTokenizer {
  ...
  static char[] DEFAULT_SEPARATOR = CharacterSet.merge(CharacterSet.BLANK,
      CharacterSet.BRACKET, CharacterSet.NEW_LINE, CharacterSet.QUOTTATION,
      new char[] { '|' });
  final static public char[] INDEX_TOKEN_SEPARATOR = CharacterSet.merge(
      DEFAULT_SEPARATOR, new char[] { '@' });
  static char[] DEFAULT_IGNORE_SEPARATOR = CharacterSet.merge(
      CharacterSet.BLANK, new char[] { '\f', '\r', '\0', 13 });
  ...
  public String next() {
    if (currPos >= buf.length)
      return null;
    while (currPos < buf.length
        && CharacterSet.isIn(buf[currPos], ignoreSeparator)) {
      currPos++;
    }
    if (currPos >= buf.length)
      return null;
